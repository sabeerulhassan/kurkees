...
import { absoluteProductImageUrl, getApiProducts, getStartingPrice, isSizeInStock } from '@/lib/api-products'
...
export default async function CollectionDetailPage({ params }: PageProps) {
  ...
  const collectionProducts = apiProducts.filter((product) => collection.productSlugs.includes(product.slug))
        ...
        <section id="matching-products" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16">
          ...
          {collectionProducts.length > 0 ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-[var(--brand-brown)]/20 bg-white p-8 text-center font-sans text-sm font-semibold text-stone-600">
              Matching product cards load from the products API. Start the API or check NEXT_PUBLIC_API_URL to show current jars here.
            </div>
          )}
        </section>
