...
export function ProductJarCluster({ className = '' }: { className?: string }) {
  return (
      ...
      {jarCluster.map((product, index) => {
        ...
        return (
          <Image
            ...
            height={360}
            priority={index === 0}
            className={`absolute drop-shadow-2xl ${positions[index]}`}
...
export function FoodMomentMosaic() {
  const moments = [
    {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      {moments.map((moment, index) => (
        <div key={moment.title} className={`relative min-h-[290px] overflow-hidden rounded-[2rem] p-6 shadow-[0_10px_0_rgba(74,44,8,.12)] ${moment.bg} ${moment.textColor}`}>
          ...
          {moment.image && (
            <Image
              src={moment.image}
              alt={`${moment.title} Kurkees jar`}
              width={230}
              height={300}
              className="absolute -bottom-10 right-0 w-[48%] max-w-[180px] rotate-[8deg] drop-shadow-2xl md:w-[58%]"
              sizes="180px"
            />
          )}
...
export function ProductStrip() {
  return (
    <div className="relative overflow-hidden bg-[var(--brand-brown)] py-4 text-white">
      <div className="flex min-w-max animate-none items-center gap-5 px-4 font-heading text-lg font-bold sm:justify-center">
        {products.slice(0, 6).map((product) => (
          <Link key={product.slug} href={`/products/${product.slug}`} className="group inline-flex items-center gap-3 rounded-full bg-white/10 py-2 pl-2 pr-5 transition-colors hover:bg-white/18">
