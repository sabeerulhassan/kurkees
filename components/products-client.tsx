...
export function ProductsClient({ initialProducts }: { initialProducts: StoreProduct[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (sort === 'price-low') list.sort((a, b) => getStartingPrice(a) - getStartingPrice(b))
    if (sort === 'price-high') list.sort((a, b) => getStartingPrice(b) - getStartingPrice(a))
  ...
  return (
    <main className="kurkees-page overflow-hidden">
      <section className="relative overflow-hidden bg-white py-12 sm:py-20">
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-8 px-4 sm:px-6 lg:grid-cols-[0.95fr_1.05fr]">
          <div className="relative z-10 text-center lg:text-left">
            ...
            <h1 className="brand-headline mt-5 text-5xl leading-[0.9] sm:text-7xl lg:text-8xl">
              Choose the jar that fits your day.
            </h1>
            <p className="brand-copy mx-auto mt-6 max-w-xl text-lg lg:mx-0">
            ...
            {initialProducts.slice(0, 4).map((product, index) => {
              ...
              return (
                getApiProductImage(product) ? (
                  <img
                    key={product.slug}
                    src={getApiProductImage(product) as string}
      ...
      <section id="price-list" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16">
            ...
            <div className="flex flex-wrap gap-2">
              {flavorFilters.map((f) => (
                <button
                  ...
                  onClick={() => setFlavor(f)}
                  className={cn(
                    'rounded-full px-4 py-2 font-sans text-xs font-extrabold uppercase tracking-[0.12em] transition-all',
                    flavor === f
